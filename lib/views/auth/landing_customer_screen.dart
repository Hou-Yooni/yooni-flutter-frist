...
class _LandingCustomerScreenState extends State<LandingCustomerScreen> {
  ...
  pickImageFromGallery() async{
    Uint8List im = await _authController.pickImage(ImageSource.gallery);  //用於從 Uint8List 獲取圖片
    setState(() {
      _image = im;
    });
  }
  ...
  pickImageFromCamera() async{
    Uint8List im = await _authController.pickImage(ImageSource.camera);  //用於從 Uint8List 獲取圖片
    setState(() {
      _image = im;
    });
  }
  ...
  signUp() async{
    ...
    String res = await _authController.signUpUsers(_fullNameController.text, _emailController.text, _passwordController.text);
    setState(() {
      isLoading = false;
    });
    if(res != 'success'){
                  ...
                  Row(
                    ...
                    children: [
                      const Text(
                        ...
                        style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'Dosis-Bold', fontStyle: FontStyle.italic ),
                      ),
                      IconButton(
                        onPressed: () {},
                        icon: const Icon(
